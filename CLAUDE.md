...
- **`UseHttpsRedirection` is deliberately absent** (see the comment in `Startup.cs`). The ACA ingress redirects HTTP→HTTPS at the edge (`allowInsecure: false`), which is what the ASP.NET Core docs recommend behind a TLS-terminating proxy; inside the container the middleware would be a no-op anyway (Kestrel is HTTP-only, so it can't determine a target HTTPS port), and if it *could* redirect it would answer health probes with a 307 — which ACA counts as success, making the probe pass even for a broken app. Browser-side protection is `UseHsts()`, and that in turn is why `ASPNETCORE_FORWARDEDHEADERS_ENABLED=true` matters: the HSTS middleware skips non-HTTPS requests, so without forwarded headers it would silently never emit the header.
