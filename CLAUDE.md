...
dotnet publish src/Web/Web.csproj --configuration Release --output ./publish/Web
...
./src/Web.Client/Run-WebApiClientsGenerator.ps1   # run with src/Web.Client as cwd
...
`Web.Client` does **not** reference Facades or below — it only references `Contracts`. It calls the API over HTTP via NSwag-generated typed clients committed at [Web.Client/_generated/WebApiClients.cs](src/Web.Client/_generated/WebApiClients.cs) (namespace `KandaEu.Volejbal.Web.Client.WebApiClients`, System.Text.Json, `GenerateDtoTypes:false` — DTOs come from `Contracts`). Clients are wired in [Web.Client/Program.cs](src/Web.Client/Program.cs) via `AddCustomizedHttpClient` with base address = the host origin (`builder.HostEnvironment.BaseAddress`), so there is no API-URL config and no CORS.

**Regenerating the API clients**: the generated file is committed (same convention as `DataLayer/_generated`). After changing controllers or DTO shapes, run `Web.Client/Run-WebApiClientsGenerator.ps1` (builds `Web`, which emits the OpenAPI document via `OpenApiGenerateDocumentsOnBuild`, then runs NSwag CLI from the tool manifest) and commit the result. This script exists because a build-time `OpenApiProjectReference` would create a circular reference (`Web.Client → Web → Web.Client`). Client-side `JsonSerializerOptions` are aligned to the server's camelCase in [Web.Client/WebApiClients/WebApiClientsJsonSettings.cs](src/Web.Client/WebApiClients/WebApiClientsJsonSettings.cs) — keep that file in sync if new clients appear.
...
- **DI registration is attribute-driven.** Mark a class `[Service]` (from `Havit.Extensions.DependencyInjection.Abstractions`) and it is picked up by `AddByServiceAttribute` in [DependencyInjection/ServiceCollectionExtensions.cs](src/DependencyInjection/ServiceCollectionExtensions.cs). Scoped lifetime is the default. Services targeted only at WebAPI use the `WebAPI` profile (see [Services/Infrastructure/ServiceProfiles.cs](src/Services/Infrastructure/ServiceProfiles.cs)).
...
- **Background jobs run via [Services/Jobs/RecurringJobsBackgroundService.cs](src/Services/Jobs/RecurringJobsBackgroundService.cs)** (plain `BackgroundService` + `PeriodicTimer`; no Hangfire, no persistence, no dashboard). At startup it runs `IEnsureTerminyJob` (non-blocking), then `EnsureTerminy` hourly. A failed run is logged and retried at the next tick. Registration is gated on a non-empty `ConnectionStrings:Database` (so OpenAPI tooling can run without a DB). The scheduler is in-process — the app must run with **max 1 replica**. `IDeaktivaceOsobJob` / [Services/DeaktivaceOsob/DeaktivaceOsobService.cs](src/Services/DeaktivaceOsob/DeaktivaceOsobService.cs) (automatic deactivation after 2 months of absence) still exist and stay DI-registered, but are **deliberately not scheduled** — deactivation is a manual action in the player-management screen. Don't "clean up" the unused job; it is kept on purpose.
...
- **Health endpoint** `/health` (path constant in [Web/Infrastructure/HealthChecks/HealthCheckEndpoints.cs](src/Web/Infrastructure/HealthChecks/HealthCheckEndpoints.cs)) exists for the ACA probes in [deploy/main.bicep](deploy/main.bicep). It deliberately registers **no** health checks — a DB check would lengthen cold start and, with `maxReplicas: 1`, turn a DB blip into a full outage. Its request telemetry is dropped by [IgnoreHealthChecksTelemetryProcessor](src/Web/Infrastructure/ApplicationInsights/IgnoreHealthChecksTelemetryProcessor.cs) (11k+ requests/day would otherwise eat the Log Analytics daily cap). The path is duplicated in `main.bicep` as `healthCheckPath` — nothing enforces that they match.
- **`UseHttpsRedirection` is wrapped in `UseWhen` so it skips `/health`** — and the reason is subtle enough that it must not be "simplified" away. In the container the middleware is a **no-op**: it needs a target HTTPS port, Kestrel is HTTP-only and no `https_port` is set, so it just logs `Failed to determine the https port for redirect` (the real HTTP→HTTPS redirect happens at the ACA ingress, `allowInsecure: false`). It is kept for local runs and non-ACA hosting. The `/health` exemption guards against it *waking up* — set `ASPNETCORE_HTTPS_PORTS` or host with an HTTPS binding and it starts answering probes with a 307, which ACA counts as success, so a completely broken app would still look healthy.
- **`ASPNETCORE_FORWARDEDHEADERS_ENABLED=true` matters for HSTS, not for the redirect**: `HstsMiddleware` returns early on `!Request.IsHttps`, so behind the TLS-terminating ingress the `Strict-Transport-Security` header would silently never be emitted without forwarded headers. It also populates the client IP that Application Insights records.
- **Cold start is only partly warmed, on purpose.** `EnsureTerminy`'s startup run in `RecurringJobsBackgroundService` builds the EF Core model and opens the connection pool, so the first request no longer pays for those. It is *not* a warmup of the request path: the job goes straight through the service, never through Kestrel, so routing, controller activation, DTO serialization metadata and — most importantly — EF Core's compiled-query cache for the *shapes* the API actually uses (`EnsureTerminy`'s LINQ shape is a different one) are all still cold. A `WarmupBackgroundService` used to fire loopback HTTP GETs at `/api/nastenka`, `/api/osoby/aktivni`, `/api/terminy` after `ApplicationStarted`; **measured: ~100 ms first request without it, ~5–40 ms with it**. It was removed as complexity not worth it here — ~100 ms per wake is acceptable for this app. Don't re-argue it from "EnsureTerminy warms the app" (it doesn't warm the HTTP path); if cold start ever matters, resurrect it via `git log -- src/Web/Infrastructure/WarmupBackgroundService.cs`.
...
- Global WebAssembly interactivity without prerender; root components (`Routes`, `HeadOutlet`) are activated from the host's `App.razor` markers — [Web.Client/Program.cs](src/Web.Client/Program.cs) registers no root components.
...
- **`TestsForLocalDebugging/`** — local-only debugging tests that hit LocalDB (`(localdb)\mssqllocaldb`). Base class [TestsForLocalDebugging/TestBase.cs](src/TestsForLocalDebugging/TestBase.cs) wires up the real DI container via `ConfigureForTests`, with `EnsureDeleted` + `Migrate` + seed between tests. Do not add these to CI.
