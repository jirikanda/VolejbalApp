...
- **Health endpoint** `/health` (path constant in [Web/Infrastructure/HealthChecks/HealthCheckEndpoints.cs](Web/Infrastructure/HealthChecks/HealthCheckEndpoints.cs)) exists for the ACA probes in [deploy/main.bicep](deploy/main.bicep). It deliberately registers **no** health checks — a DB check would lengthen cold start and, with `maxReplicas: 1`, turn a DB blip into a full outage. Two non-obvious pieces hang off it: `UseHttpsRedirection` is skipped for that path (probes arrive over plain HTTP straight to the container, so they'd get a 307 — and ACA counts 200–399 as success, which would make the probe pass even for a broken app), and its request telemetry is dropped by [IgnoreHealthChecksTelemetryProcessor](Web/Infrastructure/ApplicationInsights/IgnoreHealthChecksTelemetryProcessor.cs) (11k+ requests/day would otherwise eat the Log Analytics daily cap). The path is duplicated in `main.bicep` as `healthCheckPath` — nothing enforces that they match.
