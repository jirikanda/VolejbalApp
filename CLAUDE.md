...
- [Web/Startup.cs](Web/Startup.cs) wires MVC controllers, Razor Components (`AddInteractiveWebAssemblyComponents`), OpenAPI (built-in ASP.NET Core), rate limiting (`DefaultAPI` policy = 10/5s window with 10 queued — applied **only** to `MapControllers()`, not to Blazor/static assets), Application Insights, exception monitoring (HAVIT), and Hangfire dashboard at `/hangfire`.
