...
- [.github/workflows/build.yml](.github/workflows/build.yml) — runs on push/PR to `master`: restore → build (Release) → test → publish `Web` + `MigrationTool` artifacts for **linux-x64 with ReadyToRun** (framework-dependent). The WASM client is bundled into the host's `wwwroot/_framework`. On push to `master` or manual `workflow_dispatch` (**not** on PRs), an additional step containerizes `Web` via the .NET SDK's built-in containerization (`dotnet publish -t:PublishContainer`, no Dockerfile) and pushes `ghcr.io/<owner>/volejbal-web` tagged with the **commit SHA only** (deliberately no `latest` — this step is CI, not CD; a floating tag would be an implicit, unreviewed deploy target), authenticating with the workflow's own `GITHUB_TOKEN` (`packages: write`).
