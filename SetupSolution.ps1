...
Get-ChildItem -recurse $SolutionFolder -include *.cs,*.csproj,*.sln,*.config,*.ps1,*.json,*.tsx,*.cshtml,*.props | where { $_ -is [System.IO.FileInfo] } | where { !$_.FullName.Contains("\packages\") } | where { !$_.FullName.Contains("\obj\") } | where { !$_.Name.Equals("_SetApplicationName.ps1") } |
Foreach-Object {
