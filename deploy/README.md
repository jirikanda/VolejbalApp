...
- **Přesměrování HTTP → HTTPS dělá ingress** (`allowInsecure: false`), ne aplikace. `UseHttpsRedirection` v [Web/Startup.cs](../Web/Startup.cs) je v kontejneru **fakticky no-op** — middleware potřebuje znát cílový HTTPS port, Kestrel poslouchá jen na HTTP a žádný `https_port` nastavený není, takže jen zaloguje `Failed to determine the https port for redirect`. Zůstává kvůli lokálnímu běhu a pro případ hostování mimo ACA. Ochranu prohlížeče drží `UseHsts()`.
