...
- **Přesměrování HTTP → HTTPS dělá ingress** (`allowInsecure: false`), ne aplikace. `UseHttpsRedirection` v [Web/Startup.cs](../Web/Startup.cs) záměrně **není** — dokumentace ASP.NET Core to za reverzní proxy s TLS terminací nedoporučuje, v kontejneru by stejně nefungovalo (Kestrel poslouchá jen na HTTP, middleware by neznal cílový port) a kdyby fungovalo, odbavovalo by health probes 307 redirectem, který ACA počítá jako úspěch. Ochranu prohlížeče drží `UseHsts()`.
